...
$workloadInfoPath = Join-Path $outputPath "wgc_dirty_batch_workload.json"
$optimizedCsv = Join-Path $outputPath "wgc_dirty_batch_opt.csv"
$legacyCsv = Join-Path $outputPath "wgc_dirty_batch_legacy.csv"
...
function Read-ResultRow {
    ...
    $row = $rows | Where-Object { $_.backend -eq "wgc" -and $_.target -eq $label } | Select-Object -First 1
    if (-not $row) {
        throw "missing wgc row in $CsvPath for target label '$label'"
    }
    return $row
}
...
try {
    ...
    $legacyBatchFetchEnvName = "SNOW_CAPTURE_WGC_DISABLE_DIRTY_REGION_BATCH_FETCH"
    $hadLegacyBatchFetchValue = Test-Path ("Env:{0}" -f $legacyBatchFetchEnvName)
    $legacyBatchFetchValue = if ($hadLegacyBatchFetchValue) {
        [System.Environment]::GetEnvironmentVariable($legacyBatchFetchEnvName, "Process")
    } else {
        $null
    }
    $env:SNOW_CAPTURE_WGC_DISABLE_DIRTY_REGION_BATCH_FETCH = "1"
    try {
        Invoke-Benchmark -RegionCsv $regionCsv -SavePath $legacyCsv
    }
    finally {
        if ($hadLegacyBatchFetchValue) {
            $env:SNOW_CAPTURE_WGC_DISABLE_DIRTY_REGION_BATCH_FETCH = $legacyBatchFetchValue
        }
        else {
            Remove-Item Env:SNOW_CAPTURE_WGC_DISABLE_DIRTY_REGION_BATCH_FETCH -ErrorAction SilentlyContinue
        }
    }
    ...
    Write-Host ("A/B result vs legacy (dirty-region batch fetch disabled): avg={0:N2}% p50={1:N2}% p95={2:N2}%" -f $avgImprovementPct, $p50ImprovementPct, $p95ImprovementPct)
